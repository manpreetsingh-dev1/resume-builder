...
const [generatingIndex,setGeneratingIndex]=useState(-1)
    ...
    const generateDescription=async(index)=>{
      ...
      const prompt = [
        ...
        experience.description ? `Current description: ${experience.description}` : "",
      ]
        .filter(Boolean)
        .join(". ");

      if (!experience.position || !experience.company) {
        toast.error("Add the job title and company before using AI Enhance.");
        return;
      }

      if (!experience.description?.trim()) {
        toast.error("Add a short job description before using AI Enhance.");
        return;
      }
...
{data.map((experience,index)=>(
  <div key={index} className="p-4 border border-gray-200 rounded-lg space-y-3">
    ...
    <div className="grid md:grid-cols-2 gap-3">
     ...
     <label className="flex items-center gap-2"> <input type="checkbox" checked={experience.is_current || false} 
...
<div>
  ...
  <button  disabled={generatingIndex===index||!experience.position||!experience.company} onClick={() => generateDescription(index)} className='flex items-center gap-2 px-3 py-1 text-sm bg-purple-100 text-purple-700 rounded hover:bg-purple-200 
